...
class ProductByCategoryProvider extends ChangeNotifier {
  ...
  filterProductBySubCategory(SubCategory subCategory) {
    ...
    if (subCategory.name?.toLowerCase() == 'all') {
      filteredProduct =
          _dataProvider.products.where((element) => element.proCategoryId?.name == mySelectedSubCategory?.name).toList();
      brands = [];
    }
    else{
      filteredProduct =
  ...
  void filterProductByBrand() {
    if(selectedBrands.isEmpty) {
      filteredProduct =
          _dataProvider.products.where((product) => product.proSubCategoryId?.name == mySelectedSubCategory?.name).toList();
    } else {
      filteredProduct =
