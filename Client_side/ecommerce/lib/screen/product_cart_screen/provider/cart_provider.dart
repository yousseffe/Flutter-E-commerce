  ...
  checkCoupon() async {
    try{
      if(couponController.text.isEmpty){
        SnackBarHelper.showErrorSnackBar('Enter a coupen code');
        return;
      }
      ...
      final respone = await service.addItem(endpointUrl: 'couponCodes/check-coupon', itemData: couponData);
      if(respone.isOk){
        final ApiResponse<Coupon> apiResponse =
            ApiResponse<Coupon>.fromJson(respone.body , (json) => Coupon.fromJson(json as Map<String , dynamic>));
        if(apiResponse.success == true){
          ...
          SnackBarHelper.showErrorSnackBar('Failed to validate Coupen : ${apiResponse.message}');
        }
      } else{
        SnackBarHelper.showErrorSnackBar('Error ${respone.body?['message'] ?? respone.statusText}');
      }
      notifyListeners();
    } catch(e) {
      print(e);
